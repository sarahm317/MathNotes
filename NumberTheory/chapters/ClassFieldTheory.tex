...
\marginnote{See Theorem 5.1 in Cox's \emph{Primes of the Form $x^2 + ny^2$}.}
...
From \refthm{number-field-av} we know that any absolute value of $K$ is either equivalent to some $\mathfrak{p}$-adic absolute value or some composition map. Therefore, there is a bijection between the set
    \[\{\textrm{places of $K$}\}\]
...
The first set corresponds to finite places or nonarchimedean places. The second set corresponds to infinite places or archimedean places. Real embeddings in the second set correspond to real places while pairs of complex embeddings (paired via conujugation) correspond to complex places.
...
\begin{definition}[Ray class group, Generalized ideal class group]
    \labdef{ray-class-group}
    \labdef{generalized-ideal-class-group}
